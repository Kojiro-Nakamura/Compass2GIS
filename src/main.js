        ...
        class CompassSurveyApp {
            ...
            initConfig() {
                this.CONFIG = {
                    ...
                    defaultAttributes: [
                        { name: '蟷ｴ蠎ｦ', value: '莉､蜥・蟷ｴ蠎ｦ' }, { name: '莠区･ｭ蜷・, value: '閧ｲ謌占､・ｱ､譫玲紛蛯・ },
                        { name: '謇譛芽・錐', value: '螻ｱ逕ｰ螟ｪ驛・ }, { name: '蛯呵・, value: 'No.10' }
                    ]
                };
            }
            ...
            _initToolbarEvents() {
                bindClick(this.els.btnLoadDemo, () => {
                    this.showConfirm('迴ｾ蝨ｨ縺ｮ繝・・繧ｿ縺御ｸ頑嶌縺阪＆繧後∪縺吶ゅョ繝｢繝・・繧ｿ繧定ｪｭ縺ｿ霎ｼ縺ｿ縺ｾ縺吶°・・, () => {
            ...
            _initSettingsEvents() {
                ...
                bindClick(this.els.btnCopyClosureInfo, () => this._execCopy(this.els.closureInfo.innerText, '髢牙粋迥ｶ豕√ｒ繧ｯ繝ｪ繝・・繝懊・繝峨↓繧ｳ繝斐・縺励∪縺励◆縲・));
                bindClick(this.els.btnCopyDeclination, () => {
                    const dec = this.els.inputDeclination.value;
                    if (!dec) return this.showToast('蛛剰ｧ偵′蜈･蜉帙＆繧後※縺・∪縺帙ｓ縲・);
                    this._execCopy(dec, '蛛剰ｧ偵ｒ繧ｯ繝ｪ繝・・繝懊・繝峨↓繧ｳ繝斐・縺励∪縺励◆縲・);
                });
            }
            ...
            _initCanvasAndGlobalEvents() {
                ...
                bindClick(this.els.btnModeSelect, () => { setMode('select'); this.showToast('螟画峩縺励◆縺・ユ繧ｭ繧ｹ繝医ｄ邱壹ｒ繧ｯ繝ｪ繝・け縺励※縺上□縺輔＞'); });
                bindClick(this.els.btnModePan, () => setMode('pan'));
                bindClick(this.els.btnModeText, () => { setMode('text'); this.showToast('繧ｭ繝｣繝ｳ繝舌せ荳翫ｒ繧ｯ繝ｪ繝・け縺励※繝・く繧ｹ繝医ｒ霑ｽ蜉縺励∪縺・); });
                bindClick(this.els.btnModeLine, () => { setMode('line'); this.showToast('繧ｯ繝ｪ繝・け縺ｧ鬆らせ繧定ｿｽ蜉縲∝承繧ｯ繝ｪ繝・け縺ｧ邱壹ｒ遒ｺ螳壹＠縺ｾ縺・); });
                bindClick(this.els.btnModeErase, () => { setMode('erase'); this.showToast('蜑企勁縺励◆縺・ユ繧ｭ繧ｹ繝医ｄ邱壹ｒ繧ｯ繝ｪ繝・け縺励※縺上□縺輔＞'); });
            ...
            // 笘・慍蝗ｳ縺ｮ繧ｺ繝ｼ繝繝ｬ繝吶Ν縺ｫ髢｢繧上ｉ縺壹∫判髱｢荳翫・繝斐け繧ｻ繝ｫ繧ｵ繧､繧ｺ繧貞・驛ｨ霍晞屬(m)縺ｫ螟画鋤縺吶ｋ髢｢謨ｰ
            _getPixelsToInternalDistance(px) {
                if (this.isMapMode && this.map) {
                    const center = this.map.getCenter();
                    const point = this.map.project(center);
                    const latlng2 = this.map.unproject(point.add([0, -px])); // 荳翫↓px繝斐け繧ｻ繝ｫ遘ｻ蜍・
            ...
            _execCopy(text, successMsg) {
                if (!text) return this.showToast('繧ｳ繝斐・縺吶ｋ繝・・繧ｿ縺後≠繧翫∪縺帙ｓ縲・);
                const ta = document.createElement("textarea");
                ta.value = text; ta.style.position = "fixed"; ta.style.left = "-9999px"; document.body.appendChild(ta);
                ta.focus(); ta.select();
                try { document.execCommand('copy') ? this.showToast(successMsg) : this.showToast('繧ｳ繝斐・縺ｫ螟ｱ謨励＠縺ｾ縺励◆縲・); } 
                catch (err) { this.showToast('繧ｳ繝斐・縺ｫ螟ｱ謨励＠縺ｾ縺励◆縲・); }
                document.body.removeChild(ta);
            }

            copyAttrToClipboard() { this._execCopy(this.state.attributes.map(a => `${a.name}\t${a.value}`).join('\n'), '螻樊ｧ繝・・繧ｿ繧偵け繝ｪ繝・・繝懊・繝峨↓繧ｳ繝斐・縺励∪縺励◆縲・); }
            copyLatLonToClipboard() {
                const lat = this.els.inputLat.value, lon = this.els.inputLon.value;
                if (!lat || !lon) return this.showToast('邱ｯ蠎ｦ邨悟ｺｦ縺悟・蜉帙＆繧後※縺・∪縺帙ｓ縲・);
                this._execCopy(`${lat}, ${lon}`, '邱ｯ蠎ｦ邨悟ｺｦ繧偵け繝ｪ繝・・繝懊・繝峨↓繧ｳ繝斐・縺励∪縺励◆縲・);
            }
            copyToClipboard() { this._execCopy(this.state.tableData.map(r => r.join('\t')).join('\n'), '貂ｬ驥上ョ繝ｼ繧ｿ繧偵け繝ｪ繝・・繝懊・繝峨↓繧ｳ繝斐・縺励∪縺励◆縲・); }

            showConfirm(message, callback, title = '遒ｺ隱・) {
            ...
            clearData() {
                this.showConfirm('縺吶∋縺ｦ縺ｮ貂ｬ驥上ョ繝ｼ繧ｿ繧偵け繝ｪ繧｢縺励∪縺吶°・・, () => {
                    this.state.tableData = []; this.renderTable(); this.updateDrawing(true);
                    this.saveToLocalStorage(); this.pushState(); this.showToast('繝・・繧ｿ繧偵け繝ｪ繧｢縺励∪縺励◆縲・);
                });
            }
            ...
            openExportModal(type) {
                ...
                this.els.inputExportFileName.value = attrValues.length > 0 ? attrValues.join('_') : '莉､蜥・蟷ｴ蠎ｦ_閧ｲ謌占､・ｱ､譫玲紛蛯兩螻ｱ逕ｰ螟ｪ驛酸No.10';
                
                if (type === 'json') { 
                    this.els.exportModalTitle.textContent = '繝・・繧ｿ縺ｮ菫晏ｭ・(JSON)'; 
                    this.els.exportExtension.textContent = '.json'; 
                    this.els.htmlExportOptions.style.display = 'none'; 
                    this.els.btnApplyExport.textContent = '菫晏ｭ・;
                }
                else if (type === 'geojson') { 
                    this.els.exportModalTitle.textContent = 'GeoJSON蜃ｺ蜉・; 
                    this.els.exportExtension.textContent = '.geojson'; 
                    this.els.htmlExportOptions.style.display = 'none'; 
                    this.els.btnApplyExport.textContent = '菫晏ｭ・;
                }
                else if (type === 'html') { 
                    this.els.exportModalTitle.textContent = '蟷ｳ髱｢蝗ｳ蜃ｺ蜉幄ｨｭ螳・; 
                    this.els.exportExtension.textContent = '.html'; 
                    this.els.htmlExportOptions.style.display = 'block'; 
                    this.els.btnApplyExport.textContent = '繝励Ξ繝薙Η繝ｼ';
                }
            ...
            createRow(rowData, index) {
                ...
                chkAux.type = 'checkbox'; chkAux.checked = isAux; chkAux.title = '繝√ぉ繝・け繧貞・繧後ｋ縺ｨ菴懷峙縺九ｉ髯､螟悶＆繧後∬｣懷勧邱壹↓縺ｪ繧翫∪縺・;
            ...
            validateRow(tr) {
                ...
                if (fVal && tVal && fVal === tVal) {
                    ...
                    fromInp.title = '隴ｦ蜻・ 蝎ｨ譴ｰ轤ｹ縺ｨ隕匁ｺ也せ縺悟酔縺伜錐蜑阪〒縺・; toInp.title = '隴ｦ蜻・ 蝎ｨ譴ｰ轤ｹ縺ｨ隕匁ｺ也せ縺悟酔縺伜錐蜑阪〒縺・;
                } else {
            ...
            renderAttrTable() {
                ...
                this.state.attributes.forEach((attr, index) => {
                    ...
                    inpN.type = 'text'; inpN.value = attr.name; inpN.placeholder = '鬆・岼蜷・;
                    ...
                    inpV.type = 'text'; inpV.value = attr.value; inpV.placeholder = '蛟､';
            ...
            _renderAreaResults(complexAreas) {
                ...
                if (complexAreas.length > 0) {
                    ...
                    let html = '<div style="display: flex; justify-content: flex-start; align-items: center; gap: 8px; margin-bottom: 2px;"><div style="font-weight: bold; color: #217270; font-size: 0.75rem;">笆 蛹ｺ逕ｻ縺斐→縺ｮ髱｢遨・/div><button id="btnCopyAreaResults" class="small" style="background-color: #2E5C8A; color: white; height: 20px; font-size: 0.7rem; padding: 2px 4px;">搭 繧ｳ繝斐・</button></div><div class="area-container">';
                    let totalNetArea = 0, holeGlobalIndex = 1, copyText = '蛹ｺ逕ｻ蜷・髱｢遨・ha)\n'; 
                    ...
                    complexAreas.forEach((a, i) => {
                        ...
                        if (a.isDonut) {
                            ...
                            copyText += `蛹ｺ逕ｻ ${i + 1} (蜈ｨ菴・  ${totalHaTxt} ha\n`;
                            a.holes.forEach(h => {
                                const hAreaTxt = Utils.round4(h.area / 10000); h.globalIndex = holeGlobalIndex;
                                donutC += `  ・・髯､蝨ｰ ${h.globalIndex}  ${hAreaTxt} ha\n`; holeGlobalIndex++;
                            });
                            donutL = `<span style="font-size:0.75rem; color:#A13D44; margin-left:4px;">(蜀・髯､蝨ｰ縺ゅｊ)</span>`;
                            copyText += donutC;
                            copyText += `蛹ｺ逕ｻ ${i + 1} 蟆剰ｨ・ ${netHaTxt} ha\n`;
                        } else {
                            copyText += `蛹ｺ逕ｻ ${i + 1}  ${netHaTxt} ha\n`;
                        }
                        html += `<div class="area-item"><span>蛹ｺ逕ｻ${i+1}:</span><strong>${netHaTxt} ha</strong>${donutL}</div>`;
                        a.originalName = `蛹ｺ逕ｻ ${i + 1}`;
                    });
                    html += '</div>';
                    if (complexAreas.length > 1) {
                        const tTxt = Utils.round4(totalNetArea);
                        html += `<div class="area-total"><span>蜷郁ｨ磯擇遨・</span><span>${tTxt} ha</span></div>`; copyText += `蜷郁ｨ・${tTxt}\n`;
                    }
                    container.innerHTML = html;
                    bindClick($id('btnCopyAreaResults'), () => this._execCopy(copyText.trim(), '髱｢遨咲ｵ先棡繧偵け繝ｪ繝・・繝懊・繝峨↓繧ｳ繝斐・縺励∪縺励◆縲・));
                } else {
            ...
            updateClosureInfo(errorX, errorY, totalLength, isClosed) {
                ...
                if (totalLength === 0) { info.style.color = '#9ca3af'; info.innerHTML = `・磯哩蜷郁ｪ､蟾ｮ・・ m縲髢牙粋豈費ｼ・縲髱｢遨搾ｼ・ ha縲蜻ｨ髟ｷ・・ m・荏; return; }
                ...
                if (this.els.chkCompassAdjustment.checked && isClosed) {
                    ...
                    info.innerHTML = `・磯哩蜷郁ｪ､蟾ｮ・・{errDist.toFixed(3)}m縲髢牙粋豈費ｼ・/${Math.round(ratio).toLocaleString()}縲髱｢遨搾ｼ・{Utils.round4(Utils.calculatePolygonArea(polyPts) / 10000)}ha縲蜻ｨ髟ｷ・・{totalLength.toFixed(1)}m・荏;
                } else {
                    info.style.color = '#1f2937'; info.innerHTML = `・磯哩蜷郁ｪ､蟾ｮ・・ m縲髢牙粋豈費ｼ・縲髱｢遨搾ｼ・ ha縲蜻ｨ髟ｷ・・ m・荏;
                }
            }
            ...
            applyPasteModal() {
                ...
                if (!text) return this.showToast('繝・・繧ｿ縺悟・蜉帙＆繧後※縺・∪縺帙ｓ縲・);
                ...
                rows.forEach(rowStr => {
                    ...
                    if (cells.length >= 2) {
                        ...
                        if (cells.length > 5) {
                            ...
                            if (['true','1','陬懷勧邱・,'縺ｯ縺・].includes(val)) newRow[5] = true;
                        }
                ...
                if (hasValidData) {
                    ...
                    this.showToast('繝・・繧ｿ繧定ｲｼ繧贋ｻ倥￠縺ｾ縺励◆縲・); this.closeModal(this.els.pasteModal, this.els.pasteArea);
                } else {
                    this.showToast('譛牙柑縺ｪ繝・・繧ｿ縺瑚ｦ九▽縺九ｊ縺ｾ縺帙ｓ縺ｧ縺励◆縲ゅち繝門玄蛻・ｊ縺ｮ繝・・繧ｿ縺悟ｿ・ｦ√〒縺吶・);
                }
            }
            ...
            applyDMSModal() {
                ...
                if (lat !== null && lon !== null && !isNaN(lat) && !isNaN(lon)) {
                    ...
                    this.calculateMagDeclination(); this.showToast('蠎ｧ讓吶ｒ螟画鋤縺励※蜿肴丐縺励∪縺励◆縲・);
                    this.closeModal(this.els.dmsModal, this.els.dmsArea);
                } else { this.showToast('豁｣縺励＞蠎ｧ讓吝ｽ｢蠑上〒隱ｭ縺ｿ蜿悶ｌ縺ｾ縺帙ｓ縺ｧ縺励◆縲・); }
            }
            ...
            applyAttrPasteModal() {
                ...
                if (!text) return this.showToast('繝・・繧ｿ縺悟・蜉帙＆繧後※縺・∪縺帙ｓ縲・);
                const rows = text.split(/\r\n|\n|\r/).filter(r => r.trim() !== '');
                if (rows.length === 0) return this.showToast('譛牙柑縺ｪ繝・・繧ｿ縺後≠繧翫∪縺帙ｓ縲・);
                ...
                this.showToast('螻樊ｧ繝・・繧ｿ繧定ｲｼ繧贋ｻ倥￠縺ｾ縺励◆縲・); this.closeModal(this.els.attrPasteModal, this.els.attrPasteArea);
            }
            ...
            addDropdown(input, colIndex, rowIndex) {
                ...
                const addOption = (text, isAuto = false) => {
                    ...
                    li.innerHTML = isAuto ? `<span style="color:var(--primary);font-weight:bold;">笨ｨ ${text}</span> (閾ｪ蜍・` : text;
            ...
            _drawLabels(ctx, offsetX, offsetY, scale, detectedAreas) {
                detectedAreas.forEach((a, i) => {
                    const l1 = `蛹ｺ逕ｻ ${i + 1}`, l2 = `${Utils.round4(a.netArea / 10000)}ha`, px = offsetX + a.center.x * scale, py = offsetY - a.center.y * scale;
                    ...
                    if (a.isDonut) a.holes.forEach(h => {
                        const hl1 = `髯､蝨ｰ ${h.globalIndex}`, hl2 = `${Utils.round4(h.area / 10000)}ha`, hpx = offsetX + h.center.x * scale, hpy = offsetY - h.center.y * scale;
            ...
            // ==== Canvas縺ｨMap縺ｮ繝峨Λ繝・げ謫堺ｽ懊ｒ邨ｱ蜷医＠縺殄andleMouseMove ====
            handleMouseMove = (e) => {
                ...
                // 1. 縲仙・騾壼喧縲醍樟蝨ｨ縺ｮ繝槭え繧ｹ菴咲ｽｮ縺九ｉ縲悟・驛ｨ蠎ｧ讓吶阪ｒ險育ｮ励☆繧・
                ...
                // 6. 繧ｭ繝｣繝ｳ繝舌せ繝帙ヰ繝ｼ蛻､螳・(isMapMode 縺ｧ縺ｯ縺ｪ縺・ｴ蜷医・縺ｿ)
            ...
            initMap() {
                ...
                const to = { maxNativeZoom: 18, maxZoom: 24, attribution: "<a href='https://maps.gsi.go.jp/development/ichiran.html' target='_blank'>蝗ｽ蝨溷慍逅・劼</a>" };
                ...
                std.addTo(this.map); L.control.layers({"讓呎ｺ門慍蝗ｳ": std, "蜀咏悄・医が繝ｫ繧ｽ・・: photo, "豺｡濶ｲ蝨ｰ蝗ｳ": pale}).addTo(this.map);
            ...
            toggleMapMode() {
                ...
                if (this.isMapMode) {
                    this.els.mapContainer.style.display = 'block'; this.els.btnToggleMap.textContent = '笨擾ｸ・蝗ｳ髱｢繝薙Η繝ｼ'; this.els.btnToggleMap.classList.add('active-map');
                    ...
                    this.els.mapContainer.style.display = 'none'; this.els.btnToggleMap.textContent = '亮・・蝨ｰ蝗ｳ繝薙Η繝ｼ'; this.els.btnToggleMap.classList.remove('active-map');
                ...
                (this.state.annotations?.lines || []).forEach((line, i) => {
                    ...
                    polyline.on('mousedown', (e) => {
                        if (this.state.interactionMode === 'select') {
                            L.DomEvent.stopPropagation(e);
                            L.DomEvent.preventDefault(e.originalEvent); // 繝阪う繝・ぅ繝悶ラ繝ｩ繝・げ繧帝亟豁｢
                            this.map.dragging.disable();
                            
                            document.body.classList.add('left-dragging'); // 繧ｯ繝ｪ繝・け縺励◆迸ｬ髢薙↓繧ｰ繝ｼ縺ｫ縺吶ｋ
            ...
            _drawMapNodesAndLabels(lat0, lon0, lonDPM) {
                ...
                this.state.detectedAreas.forEach((a, i) => {
                    L.marker([lat0 + a.center.y * CONSTANTS.LAT_DEG_PER_METER, lon0 + a.center.x * lonDPM], { icon: L.divIcon({ className: 'map-label-container', html: `<div class="area-map-label">蛹ｺ逕ｻ ${i + 1}<br><span style="font-weight:normal; font-size:1em;">${Utils.round4(a.netArea / 10000)}ha</span></div>`, iconSize: [0, 0], iconAnchor: [0, 0] }), interactive: false }).addTo(this.mapLayerGroup);
                    if (a.isDonut) a.holes.forEach(h => L.marker([lat0 + h.center.y * CONSTANTS.LAT_DEG_PER_METER, lon0 + h.center.x * lonDPM], { icon: L.divIcon({ className: 'map-label-container', html: `<div class="hole-map-label">髯､蝨ｰ ${h.globalIndex}<br><span style="font-weight:normal; font-size:1em;">${Utils.round4(h.area / 10000)}ha</span></div>`, iconSize: [0, 0], iconAnchor: [0, 0] }), interactive: false }).addTo(this.mapLayerGroup));
                });
            }
            ...
            exportGeoJSON(fileName) {
                ...
                if (isNaN(lat0) || isNaN(lon0)) return this.showToast("蝓ｺ貅也せ(B.P.)縺ｮ邱ｯ蠎ｦ繝ｻ邨悟ｺｦ繧呈ｭ｣縺励￥蜈･蜉帙＠縺ｦ縺上□縺輔＞縲・);
                ...
                features.push({ type: "Feature", properties: { "貂ｬ轤ｹ蜷・: fp, "邱ｯ蠎ｦ": parseFloat(lat0.toFixed(6)), "邨悟ｺｦ": parseFloat(lon0.toFixed(6)), ...customProps }, geometry: { type: "Point", coordinates: [lon0, lat0] } });
                this.state.nodes.forEach((node, name) => {
                    if (name === fp && Math.abs(node.x) < 0.001 && Math.abs(node.y) < 0.001) return;
                    features.push({ type: "Feature", properties: { "貂ｬ轤ｹ蜷・: name, "邱ｯ蠎ｦ": parseFloat((lat0 + node.y * CONSTANTS.LAT_DEG_PER_METER).toFixed(6)), "邨悟ｺｦ": parseFloat((lon0 + node.x * lonDPM).toFixed(6)), ...customProps }, geometry: { type: "Point", coordinates: [lon0 + node.x * lonDPM, lat0 + node.y * CONSTANTS.LAT_DEG_PER_METER] } });
                });
                ...
                if (this.els.chkCompassAdjustment.checked) {
                    this.state.detectedAreas.forEach(p => {
                        ...
                        features.push({ type: "Feature", properties: { name: p.originalName, "蜈ｨ菴馴擇遨・m2)": parseFloat(p.area.toFixed(2)), "髯､蝨ｰ髱｢遨・m2)": parseFloat((p.area - p.netArea).toFixed(2)), "豁｣蜻ｳ髱｢遨・m2)": parseFloat(p.netArea.toFixed(2)), "蜈ｨ菴馴擇遨・ha)": parseFloat(Utils.round4(p.area / 10000)), "髯､蝨ｰ髱｢遨・ha)": parseFloat(Utils.round4((p.area - p.netArea) / 10000)), "豁｣蜻ｳ髱｢遨・ha)": parseFloat(Utils.round4(p.netArea / 10000)), "蜻ｨ髟ｷ(m)": parseFloat(p.perimeter.toFixed(2)), "讒矩": p.isDonut ? `繝峨・繝翫ヤ繝昴Μ繧ｴ繝ｳ・・{p.holes.length}縺､縺ｮ遨ｴ・荏 : "騾壼ｸｸ繝昴Μ繧ｴ繝ｳ", ...customProps }, geometry: { type: "Polygon", coordinates: coords } });
                    });
                }
                this.state.points.forEach(p => {
                    features.push({ type: "Feature", properties: { "繧ｿ繧､繝・: p.type === 'main' ? '譛ｬ邱・ : '謾ｯ邱・, "菴懷峙蟇ｾ雎｡": p.isDraw ? '縺ｯ縺・ : '縺・＞縺・, "蝎ｨ譴ｰ轤ｹ": p.fromName, "隕匁ｺ也せ": p.toName, "譁ｹ菴崎ｧ・: parseFloat(p.input.az.toFixed(2)), "鬮倅ｽ手ｧ・: p.input.el, "譁懆ｷ晞屬": p.input.sd, "豌ｴ蟷ｳ霍晞屬": parseFloat(p.input.hd.toFixed(2)), ...customProps }, geometry: { type: "LineString", coordinates: [[lon0 + p.fromX * lonDPM, lat0 + p.fromY * CONSTANTS.LAT_DEG_PER_METER], [lon0 + p.toX * lonDPM, lat0 + p.toY * CONSTANTS.LAT_DEG_PER_METER]] } });
                });
                if (this.state.annotations) {
                    this.state.annotations.texts.forEach(t => features.push({ type: "Feature", properties: { "繧ｿ繧､繝・: "豕ｨ險・, "繝・く繧ｹ繝・: t.text, "譁・ｭ苓牡": t.color || '#059669', "繧ｵ繧､繧ｺ": t.fontSize || 14, "蝗櫁ｻ｢隗貞ｺｦ": parseFloat(((t.rotation || 0) * 180 / Math.PI).toFixed(2)), ...customProps }, geometry: { type: "Point", coordinates: [lon0 + t.x * lonDPM, lat0 + t.y * CONSTANTS.LAT_DEG_PER_METER] } }));
                    this.state.annotations.lines.forEach(l => features.push({ type: "Feature", properties: { "繧ｿ繧､繝・: "騾｣邯夂ｷ・, "蝗櫁ｻ｢隗貞ｺｦ": parseFloat(((l.rotation || 0) * 180 / Math.PI).toFixed(2)), ...customProps }, geometry: { type: "LineString", coordinates: l.points.map(pt => [lon0 + pt.x * lonDPM, lat0 + pt.y * CONSTANTS.LAT_DEG_PER_METER]) } }));
                }

                if (!fileName) fileName = '莉､蜥・蟷ｴ蠎ｦ_閧ｲ謌占､・ｱ､譫玲紛蛯兩螻ｱ逕ｰ螟ｪ驛酸No.10';
            ...
            showHTMLPreview(fileName, pSize = 'A4', ori = 'landscape', sOpt = 'auto', split = 50, existingWin = null) {
                ...
                // SVG・郁レ譎ｯ縺ｨ縺ｪ繧狗ｷ壹ｄ髱｢縲√ラ繝・ヨ縺ｮ縺ｿ蜃ｺ蜉帙☆繧具ｼ峨→縲√ラ繝ｩ繝・げ蜿ｯ閭ｽ縺ｪ繝・く繧ｹ繝育ｾ､・・TML・峨ｒ逕滓・
                ...
                // 蝨ｰ蝗ｳ繝薙Η繝ｼ逕ｨ縺ｮ邱ｯ蠎ｦ邨悟ｺｦ蠅・阜繧定ｨ育ｮ・
                // 蝨ｰ蝗ｳ繝薙Η繝ｼ逕ｨ縺ｮ邱ｯ蠎ｦ邨悟ｺｦ蠅・阜繧定ｨ育ｮ・(printMapBg 縺檎畑邏吝・菴薙→蜷後§繧ｵ繧､繧ｺ縺ｫ縺ｪ繧九◆繧・
                ...
                const htmlContent = `<!DOCTYPE html><html lang="ja"><head><meta charset="UTF-8"><title>蟷ｳ髱｢蝗ｳ (${pSize} ${ori})</title>
...
<div class="instruction" id="toolbar">
    <div style="display: flex; align-items: center; gap: 15px;">
        <span>庁 蝗ｳ髱｢蜈ｨ菴薙√∪縺溘・陦ｨ縲∵枚蟄苓ｦ∫ｴ繧貞句挨縺ｫ繝峨Λ繝・げ縺励※閾ｪ逕ｱ縺ｫ遘ｻ蜍輔・隱ｿ謨ｴ縺ｧ縺阪∪縺吶・/span>
        <div style="display: flex; align-items: center; gap: 6px; background: rgba(0,0,0,0.2); padding: 4px 10px; border-radius: 6px;">
            <span style="font-size: 0.85rem;">陦ｨ遉ｺ蛟咲紫:</span>
            <button class="btn-zoom" id="btnZoomOut" title="邵ｮ蟆・>・・/button>
            <span id="zoomLevel" style="font-size: 0.9rem; min-width: 45px; text-align: center; font-weight: bold;">100%</span>
            <button class="btn-zoom" id="btnZoomIn" title="諡｡螟ｧ">・・/button>
            <button class="btn-zoom" id="btnFitScreen" title="逕ｻ髱｢縺ｫ蜷医ｏ縺帙ｋ" style="width: auto; padding: 0 8px; font-size: 0.8rem; margin-left: 4px;">笵ｶ 繝輔ぅ繝・ヨ</button>
            <label style="font-size:13px; margin-left: 10px; cursor: pointer; color: white;"><input type="checkbox" id="chkBgMap" ${showMapBg ? 'checked' : ''}> 閭梧勹蝨ｰ蝗ｳ</label>
            <select id="bgMapType" style="margin-left:5px; font-size:13px; padding: 2px;">
                <option value="https://cyberjapandata.gsi.go.jp/xyz/std/{z}/{x}/{y}.png" ${currentTileUrl.includes('std') ? 'selected' : ''}>讓呎ｺ門慍蝗ｳ</option>
                <option value="https://cyberjapandata.gsi.go.jp/xyz/seamlessphoto/{z}/{x}/{y}.jpg" ${currentTileUrl.includes('seamlessphoto') ? 'selected' : ''}>蜀咏悄</option>
                <option value="https://cyberjapandata.gsi.go.jp/xyz/pale/{z}/{x}/{y}.png" ${currentTileUrl.includes('pale') ? 'selected' : ''}>豺｡濶ｲ蝨ｰ蝗ｳ</option>
            </select>
        </div>
        
        <div style="display: flex; align-items: center; gap: 6px; background: rgba(0,0,0,0.2); padding: 4px 10px; border-radius: 6px;">
            <span style="font-size: 0.85rem;">逕ｨ邏・</span>
            ...
            <select id="plOrientation" style="font-size:12px; padding:2px;">
                <option value="landscape" ${ori==='landscape'?'selected':''}>讓ｪ</option>
                <option value="portrait" ${ori==='portrait'?'selected':''}>邵ｦ</option>
            </select>
            <span style="font-size: 0.85rem; margin-left: 6px;">邵ｮ蟆ｺ:</span>
            <select id="plScale" style="font-size:12px; padding:2px;">
                <option value="auto" ${sOpt==='auto'?'selected':''}>閾ｪ蜍慕ｸｮ蟆ｺ</option>
            ...
            <span style="font-size: 0.85rem; margin-left: 6px;">謌先棡陦ｨ謚倩ｿ・</span>
            <input type="number" id="plTableSplit" value="${split}" style="width:45px; font-size:12px; padding:2px;">
        </div>
    </div>
    <div>
        <button class="btn" onclick="window.print()">蜜・・蜊ｰ蛻ｷ</button>
        <button class="btn" id="btnSaveDXF" style="background: #c2410c;">沈 DXF菫晏ｭ・/button>
        <button class="btn btn-save" id="btnSaveHTML">沈 HTML菫晏ｭ・/button>
    </div>
</div>
<div class="page-wrapper">
    <div class="page-container">
        ...
        ${this.els.closureInfo.innerText ? `<div class="closure-info draggable" style="bottom:15mm; left:15mm; padding:5px; font-size:9pt;">髢牙粋迥ｶ豕・ ${this.els.closureInfo.innerText}</div>` : ''}
    </div>
</div>
<script>
window.addEventListener('load', () => {
    ...
    function applyPreviewSettings() {
        if (window.opener && window.opener.app && typeof window.opener.app.showHTMLPreview === 'function') {
            ...
            alert('蜈・・逕ｻ髱｢縺碁哩縺倥ｉ繧後※縺・ｋ縺九√い繧ｯ繧ｻ繧ｹ縺ｧ縺阪↑縺・◆繧∬ｨｭ螳壹ｒ蜿肴丐縺ｧ縺阪∪縺帙ｓ縲・);
        }
    }
...
<\/script></body></html>`;
            ...
            _buildExportHTMLResultsTable(splitRows) {
                ...
                data.forEach((r, i) => {
                    ...
                    if ((splitRows > 0 && (i + 1) % splitRows === 0) || i === data.length - 1) {
                        if (i === data.length - 1) curHtml += `<tr style="border-top:2px solid #000;"><td colspan="3" style="text-align:center;">蜷・險・/td><td style="text-align:right;">${tSD.toFixed(2)}</td><td style="text-align:right;">${tHD.toFixed(2)}</td></tr>`;
                        html += `<table style="width:85mm; font-size:8pt; border-collapse:collapse; margin-left:10px;"><thead><tr><th>貂ｬ 轤ｹ</th><th>譁ｹ菴崎ｧ・/th><th>鬮倅ｽ手ｧ・/th><th>譁懆ｷ晞屬</th><th>豌ｴ蟷ｳ霍晞屬</th></tr></thead><tbody>${curHtml}</tbody></table>`; curHtml = '';
                    }
                });
                return `<div class="result-table-wrapper draggable"><div style="font-weight:bold;text-align:center;border-bottom:1px solid #000;margin-bottom:5px;">謌・譫・陦ｨ</div><div style="display:flex; justify-content:flex-end;">${html}</div></div>`;
            }
            ...
            _buildExportHTMLAttrTable(scTxt, lat, lon, dec) {
                ...
                return html + `<tr><th style="text-align:left;">邵ｮ蟆ｺ</th><td style="font-weight:bold;">${scTxt}</td></tr><tr><th style="text-align:left;">蝓ｺ貅也せ</th><td style="font-size:8pt;">Lat ${lat} / Lon ${lon} <br>(蛛剰ｧ・ ${dec}蠎ｦ)</td></tr>`;
            }
            ...
            _buildExportHTMLAreaTable() {
                ...
                this.state.detectedAreas.forEach((a, i) => {
                    ...
                    if (a.isDonut) {
                        ...
                        html += `<tr><td style="border-bottom-style: dashed; border-bottom-color: #999;">蛹ｺ逕ｻ ${i + 1} (蜈ｨ菴・</td><td style="text-align:right; border-bottom-style: dashed; border-bottom-color: #999;">${totalHa} ha</td></tr>`;
                        a.holes.forEach(h => {
                            const hHa = Utils.round4(h.area / 10000);
                            html += `<tr><td style="border-bottom-style: dashed; border-bottom-color: #999; color: #A13D44; padding-left: 12px;">・・髯､蝨ｰ ${h.globalIndex || ''}</td><td style="text-align:right; border-bottom-style: dashed; border-bottom-color: #999; color: #A13D44;">${hHa} ha</td></tr>`;
                        });
                        html += `<tr><td>蛹ｺ逕ｻ ${i + 1} 蟆剰ｨ・/td><td style="text-align:right; font-weight: bold;">${netHa} ha</td></tr>`;
                    } else {
                        html += `<tr><td>蛹ｺ逕ｻ ${i + 1}</td><td style="text-align:right;">${netHa} ha</td></tr>`;
                    }
                });
                return `<div class="area-table-wrapper draggable"><div style="font-weight:bold;text-align:center;border-bottom:1px solid #000;margin-bottom:5px;">髱｢ 遨・陦ｨ</div><table style="width:100%;font-size:9pt;">${html}<tr style="border-top:2px solid #000;font-weight:bold;"><td>蜷・險・/td><td style="text-align:right;">${Utils.round4(tArea)} ha</td></tr></table></div>`;
            }
            ...
            _generateDraggableLabelsHTML(scale, ox, oy, pxPerMm, tx, ty) {
                ...
                // 隕ｪ繧ｳ繝ｳ繝・リ・・ap-group・峨・襍ｷ轤ｹ繧ｪ繝輔そ繝・ヨ(mm)
                ...
                // 2. 髱｢遨阪Λ繝吶Ν
                this.state.detectedAreas.forEach((a, i) => {
                    const l1 = `蛹ｺ逕ｻ ${i+1}`, l2 = `${Utils.round4(a.netArea/10000)}ha`;
                    ...
                    if (a.isDonut) {
                        a.holes.forEach(h => {
                            const hl1 = `髯､蝨ｰ ${h.globalIndex}`, hl2 = `${Utils.round4(h.area/10000)}ha`;
            ...
            exportJSON(fileName) {
                ...
                if (!fileName) fileName = '莉､蜥・蟷ｴ蠎ｦ_閧ｲ謌占､・ｱ､譫玲紛蛯兩螻ｱ逕ｰ螟ｪ驛酸No.10'; if (!fileName.endsWith('.json')) fileName += '.json';
                this._downloadFile("data:text/json;charset=utf-8," + encodeURIComponent(JSON.stringify(st)), fileName);
                this.showToast('JSON繝輔ぃ繧､繝ｫ縺ｨ縺励※菫晏ｭ倥＠縺ｾ縺励◆縲・);
            }
            ...
            async processJsonFiles(jsonFiles) {
                if (jsonFiles.length === 0) { this.showToast('JSON繝輔ぃ繧､繝ｫ縺瑚ｦ九▽縺九ｊ縺ｾ縺帙ｓ縺ｧ縺励◆縲・); if (this.els.inputFileJSON) this.els.inputFileJSON.value = ''; return; }
                ...
                this.els.importPreviewModal.style.display = 'flex'; this.els.importNoPreviewText.textContent = '隱ｭ縺ｿ霎ｼ縺ｿ荳ｭ...';
                ...
                if (this.importFilesList.length === 0) { this.els.importNoPreviewText.textContent = '譛牙柑縺ｪ繝・・繧ｿ縺瑚ｦ九▽縺九ｊ縺ｾ縺帙ｓ縺ｧ縺励◆縲・; return; }
            ...
            selectImportFile(index) {
                ...
                else { this.els.importPreviewImage.style.display = 'none'; this.els.importNoPreviewText.style.display = 'inline-block'; this.els.importNoPreviewText.textContent = '繝励Ξ繝薙Η繝ｼ逕ｻ蜒上↑縺・; }

                let attrText = item.data.attributes ? item.data.attributes.filter(a=>a.value).map(a=>a.value).join(' / ') : '';
                this.els.importPreviewInfo.innerHTML = `<strong>繝輔ぃ繧､繝ｫ蜷・</strong> ${item.name}<br>` + (item.data.tableData ? `<strong>繝・・繧ｿ謨ｰ:</strong> ${item.data.tableData.length}陦・br>` : '') + (attrText ? `<strong>螻樊ｧ諠・ｱ:</strong> <span style="color:#4b5563;">${attrText}</span>` : '');
            ...
            applyImportData(saved) {
                ...
                this.pushState(); this.showToast('繝・・繧ｿ繧定ｪｭ縺ｿ霎ｼ縺ｿ縺ｾ縺励◆縲・);
            }
        }
